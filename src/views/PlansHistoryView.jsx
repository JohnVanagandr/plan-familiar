...
import { FileText, ArrowRight, HouseHeart } from "lucide-react";
...
export const PlansHistoryView = () => {
  ...
  const {
    ...
    navegarPagina,
    hayMultiplesPaginas,
  } = usePaginacion("familyPlans", true);
            ...
            {planes.map((plan) => (
              <Card key={plan.id} padding="md" className="flex flex-col gap-4 relative">
                <div className="flex items-center justify-between z-10">
                  ...
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${estadoColor[plan.status] ?? "bg-slate-100 text-slate-500"}`}>
                    {plan.status}
                  </span>
                </div>
